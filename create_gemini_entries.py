...
def generate_entry_with_gemini(topic: str, template: str, api_key: str) -> Optional[str]:
    """Generate a MediaWiki entry for the given topic using Gemini API."""
    
    # Construct the prompt
    prompt = f"""Use the supplied template to create a brief entry on the given subject using mediawiki syntax.\ 
    Make sure to use links to important concepts in the entry text, like e.g. [[psychology]].\
...
Please create a MediaWiki entry for this subject following the template structure."""
...
def main():
    parser = argparse.ArgumentParser(description='Generate MediaWiki entries using Gemini API')
    parser.add_argument('-template', required=True, help='Path to the entry template file')
    ...
    for topic in topics:
        ...
        entry = generate_entry_with_gemini(topic, template, api_key)
